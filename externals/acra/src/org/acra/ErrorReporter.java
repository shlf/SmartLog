...
public class ErrorReporter implements Thread.UncaughtExceptionHandler {
            ...
            ApplicationHelper.registerActivityLifecycleCallbacks(context,
                    new ActivityLifecycleCallbacksCompat() {
                        ...
                        public void onActivitySaveInstanceState(Activity activity, Bundle outState) {
                            if (ACRA.DEV_LOGGING)
                                ACRA.log.i(ACRA.LOG_TAG,
                                        "onActivitySaveInstanceState " + activity.getClass());
                        }
                        ...
                        public void onActivityDestroyed(Activity activity) {
                            if (ACRA.DEV_LOGGING)
                                ACRA.log.i(ACRA.LOG_TAG,
                                        "onActivityDestroyed " + activity.getClass());
                        }
                    });
        }
     ...
     *             {@link ACRA#init(android.app.Application)} has not yet been
     *             called.
     ...
     * 
     * @param sender ReportSender to set as the sole sender for this
     *            ErrorReporter.
     */
        ...
        try {
            ...
            if (!enabled) {
                if (mDfltExceptionHandler != null) {
                    ...
                    Log.e(ACRA.LOG_TAG, "ACRA caught a " + e.getClass().getSimpleName() + " for "
                            + mContext.getPackageName(), e);
                }
                return;
            }
    ...
    private void endApplication() {
        if (ACRA.getConfig().mode() == ReportingInteractionMode.SILENT
            ...
            if (mDfltExceptionHandler != null) {
                ...
                Log.w(ACRA.LOG_TAG,
                        "NOT Handing Exception on to default ExceptionHandler - non configured");
            }
        } else {
            ...
            Log.e(LOG_TAG,
                    mContext.getPackageName() + " fatal error : " + unhandledThrowable.getMessage(),
                    unhandledThrowable);
    ...
    public void setEnabled(boolean enabled) {
        Log.i(ACRA.LOG_TAG,
                "ACRA is " + (enabled ? "enabled" : "disabled") + " for "
    ...
    SendWorker startSendingReports(boolean onlySendSilentReports, boolean approveReportsFirst) {
        final SendWorker worker = new SendWorker(mContext, mReportSenders, onlySendSilentReports,
                approveReportsFirst, ACRA.getConfig().savePath());
        worker.start();
        return worker;
    }
    ...
    public void checkReportsOnApplicationStart() {
        ...
        if ((ACRA.getConfig().mode() == ReportingInteractionMode.NOTIFICATION || ACRA.getConfig()
        ...
        String[] filesList = reportFinder.getCrashReportFiles(ACRA.getConfig().savePath());
        ...
        if (filesList != null && filesList.length > 0) {
            ...
            filesList = reportFinder.getCrashReportFiles(ACRA.getConfig().savePath());
            ...
            if (reportingInteractionMode == ReportingInteractionMode.SILENT
                ...
                if (reportingInteractionMode == ReportingInteractionMode.TOAST
                    ...
                    // non-silent reports.
                    ToastSender.sendToast(mContext, ACRA.getConfig().resToastText(),
                            Toast.LENGTH_LONG);
                }
     ...
     * @param e The {@link Throwable} to be reported. If null the report will
     *            contain a new Exception("Report requested by developer").
     * @param endApplication Set this to true if you want the application to be
     *            ended after sending the report.
     */
     ...
     * @param forceSilentReport This report is to be sent silently, whatever
     *            mode has been configured.
     * @param endApplication Whether to end the application once the error has
     *            been handled.
     */
    private void handleException(Throwable e, ReportingInteractionMode reportingInteractionMode,
        ...
        try {
            ...
            Log.d(ACRA.LOG_TAG, "Failed to initlize " + exceptionHandlerInitializer
                    + " from #handleException");
        }
        ...
        final CrashReportData crashReportData = crashReportDataFactory.createCrashData(e,
                forceSilentReport, brokenThread);

        // Always write the report file

        final String reportFileName = getReportFileName(crashReportData);
        saveCrashReportFile(reportFileName, crashReportData, ACRA.getConfig().savePath());
     ...
     * 
     * @param reportFileName Name fo the error report to display in the crash
     *            report dialog.
     */
    ...
    private void notifySendReport(String reportFileName) {
        ...
        final PendingIntent contentIntent = PendingIntent.getActivity(mContext,
                mNotificationCounter++, notificationIntent, PendingIntent.FLAG_UPDATE_CURRENT);
     ...
     *            data are used.
     */
    private void saveCrashReportFile(String fileName, CrashReportData crashData, String path) {
        try {
            Log.d(LOG_TAG, "Writing crash report file name: " + fileName + ", " + " path: " + path);
            ...
            persister.store(crashData, fileName, path);
        } catch (Exception e) {
    ...
    private void deletePendingReports(boolean deleteApprovedReports,
        ...
        final CrashReportFinder reportFinder = new CrashReportFinder(mContext);
        final String[] filesList = reportFinder.getCrashReportFiles(ACRA.getConfig().savePath());
        ...
        for (int iFile = 0; iFile < filesList.length - nbOfLatestToKeep; iFile++) {
            ...
            if ((isReportApproved && deleteApprovedReports)
                ...
                if (TextUtils.isEmpty(ACRA.getConfig().savePath())) {
                    fileToDelete = new File(mContext.getFilesDir(), fileName);
                } else {
                    fileToDelete = new File(ACRA.getConfig().savePath(), fileName);
                }
    ...
    public void setDefaultReportSenders() {
        ...
        if (conf.formUri() != null && !"".equals(conf.formUri())) {
            setReportSender(new HttpSender(ACRA.getConfig().httpMethod(), ACRA.getConfig()
                    .reportType(), null));
            return;
        }
