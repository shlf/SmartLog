...
/**
 * This appender logs messages to a file using JSR75 (FileConnection) The
 * appender tracks the file size and if it exceeds a given maximum size
 * (customizable by clients) then the current log file is renamed appending a
 * .old to the log name and a new one is created. Therefore the maximum size on
 * this is about 2 times the maxFileSize (this is not accurate as there is no
 * limit on the size of the single message printed).
 */
public class FileAppender implements Appender {
     ...
     * files whose size is (roughly) bound to maxFileSize. The minimum file size
     * is 1024 as smaller size does not really make sense. If a client needs
     * smaller files it should probably the usage of other Appenders or modify
     * the behavior of this one by deriving it.
     * 
     ...
     * behavior. Note that regardless of this setting, the log is always written
     * to a file.
     */
        ...
        synchronized (lock) {
            try {
                if (os != null) {
                    ...
                    // If the file grows beyond the limit, we rename it and
                    // create a new
                    // one
                ...
                e.printStackTrace();
                // We try to close and reopen the log file. The message being
                // logged
                ...
                try {
                    ...
                    // We cannot even close the file, too bad. Logging maybe
                    // disabled
    ...
    /**
     * Perform additional actions needed when setting a new level. FileAppender
    ...
    public LogContent getLogContent() throws IOException {
        synchronized (lock) {
            ...
            try {
                ...
                // to read it. Note that this method is synchronized on so so
                // that we
                // can safely do it
                os.close();
                file.close();
